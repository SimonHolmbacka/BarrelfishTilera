...
# error Must define a kernel scheduling policy!
    ...
    DECL(DISP_CRIT_PC_LOW, struct dispatcher_shared_tilepro, crit_pc_low);
    DECL(DISP_CRIT_PC_HIGH, struct dispatcher_shared_tilepro, crit_pc_high);
    DECL(DISP_ASID, struct dispatcher_shared_tilepro, asid);
    DECL(DISP_ENABLED_AREA, struct dispatcher_shared_tilepro, enabled_save_area);
    DECL(DISP_DISABLED_AREA, struct dispatcher_shared_tilepro, disabled_save_area);
    DECL(DISP_TRAP_AREA, struct dispatcher_shared_tilepro, trap_save_area);
    DECL(DISP_GENERIC, struct dispatcher_tilepro, generic);
#endif
    ...
    ASSERT(sizeof(struct dispatcher_tilepro) <= (1 << DISPATCHER_FRAME_BITS));
#else
