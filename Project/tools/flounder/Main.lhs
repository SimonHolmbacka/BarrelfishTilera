...
>     where arch = optArch opts
> generator _ UDN_Header = UDN.header
> generator opts UDN_Stub
>     | isNothing arch = error "no architecture specified for UDN stubs"
>     | otherwise = UDN.stub (fromJust arch)
>     where arch = optArch opts
...
>             Option [] ["udn-header"] (NoArg $ addTarget UDN_Header) "Create a header file for UDN",
>             Option [] ["udn-stub"] (NoArg $ addTarget UDN_Stub)     "Create a stub file for UDN",
