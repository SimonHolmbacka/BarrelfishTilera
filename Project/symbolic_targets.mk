...
# TilePro-specific modules to build by default
# Kernel and bootloader are build and bundled together in the simulate-phase.
MODULES_tilepro=\
	sbin/bootloader \
	sbin/cpu \
	sbin/fish \
...
else ifeq ($(ARCH),tilepro)
	GDB=tile-gdb
	#TILE_RUN_START=tile-monitor --simulator --functional --console --config tile64
	TILE_RUN_START=tile-monitor --simulator --functional --console --config 3x3
	#TILE_RUN_START=tile-monitor --net 192.168.11.4 --config 6x1
	TILE_RUN_END=--sim-args -+- --symbols tilepro/sbin/bootloader --symbols tilepro/sbin/cpu -+- --wait --bootrom-file tilepro/sbin/cpu.bin
# The requirement of cpu.bin triggers the bundling of the bootrom, which contains the bootloader, kernel & all modules.
# Must be done here, so we can add all modules as requirement, otherwise they could not be compiled, yet.
simulate: $(MODULES) tilepro/sbin/cpu.bin
	$(TILE_RUN_START) $(TILE_RUN_END)
.PHONY : simulate
 
debugsim: $(MODULES) tilepro/sbin/cpu.bin
	$(TILE_RUN_START) --gdb-port 11111 --bm-debug-on-panic $(TILE_RUN_END)
.PHONY : debugsim
endif
...
# here we use our shell script to create module list from menu.lst, and then call tile-mkboot, in the simulation phase
tilepro/sbin/cpu.bin: $(SRCDIR)/tools/tilepro/mkboot-menulist.sh $(SRCDIR)/kernel/arch/tilepro/tilepro-conf.hvc $(ARCH)/sbin/bootloader $(ARCH)/menu.lst $(MODULES)
	@$(SRCDIR)/tools/tilepro/mkboot-menulist.sh $(ARCH)/menu.lst $(SRCDIR)/kernel/arch/tilepro/tilepro-conf.hvc $(ARCH)/sbin/bootloader $@.tmp
	@$(SRCDIR)/tools/tilepro/tilepro_check_bootloader_integrity.sh $(ARCH)/sbin/bootloader
# To make sure that when the check crashes, a new execution of make, would not ignore the check
	mv $@.tmp $@
