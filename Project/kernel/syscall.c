                     ...
                     capaddr_t vptr, capaddr_t dptr, bool run, capaddr_t odptr)
{
    ...
    if (vptr != CPTR_NULL) {
        ...
        case ObjType_VNode_tilepro_l1:
        	dcb->vspace = (lvaddr_t)gen_phys_to_local_phys(vroot->u.vnode_tilepro_l1.base);
        	break;
...
// here we change the flags' type to 64-bit
struct sysret sys_map(struct capability *ptable, cslot_t slot, capaddr_t source_cptr, int source_vbits, uint64_t flags, uintptr_t offset, uintptr_t pte_count)
#else
struct sysret
