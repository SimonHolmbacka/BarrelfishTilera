                         ...
                         alloc_phys_func alloc_phys, lvaddr_t *retparamaddr)
{
    ...
    for (int i = 0; i < argc; i++) {
        ...
        // TODO The casts are necessary, because lvaddr_t and lpaddr_t may have different sizes in bytes.
        // If this is too annoying, just change the calculation to one, which is more obvious.
        params->argv[i] = (void *)(args_base + (lvaddr_t)mem_to_local_phys((lvaddr_t)buf)
                                   - (lvaddr_t)st->args_page);
        strcpy(buf, argv[i]);
        buf += arglen + 1;
        buflen -= arglen + 1;
    }
    ...
    *retparamaddr = args_base;
    ...
    // here we cannot use the default setting in the loop above to create ram type,
    // because our page size is too large, our ram will be running out.
	// On tilepro the former calculation resulted in 1024 pages with each 64k, which are 64M.
	// So here we allocate the same number as other arch allocated, which is 64.
    int base_page_num = 0;
#ifdef __tilepro__
    base_page_num = 64;
#else
