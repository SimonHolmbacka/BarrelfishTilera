...
-- Maybe someone wants to provide his own libc/newlib,
-- without using the one in this tree, so there is an "own" option
libc :: String
--libc = "oldc"
--libc = "ownoldc"
libc = "newlib"
...
defines = [ Str ("-D" ++ d) | d <- [  
             ...
             if libc == "oldc" then "CONFIG_OLDC" else "",
             if libc == "ownoldc" then "CONFIG_OLDC" else "",
             if libc == "newlib" then "CONFIG_NEWLIB" else "",
             if libc == "ownnewlib" then "CONFIG_NEWLIB" else "",
             if oneshot_timer then "CONFIG_ONESHOT_TIMER" else "",
             if use_kaluga_dvm then "USE_KALUGA_DVM" else ""
             ], d /= "" ]

--Maybe we want to provide a prebuilt libc/newlib? Don't know yet, but maybe...

 -- Sets the include path for the libc
libcInc :: String
libcInc | libc == "oldc" = "/include/oldc"
        | libc == "newlib" = "/lib/newlib/newlib/libc/include"
        | libc == "ownoldc" = ""
        | libc == "ownnewlib" = ""
        | otherwise = ""
...
source_dir = "/home/andreas/barrelfish/new_test2"
