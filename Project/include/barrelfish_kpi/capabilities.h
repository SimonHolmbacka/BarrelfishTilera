...
static inline bool type_is_vnode(enum objtype type)
{
    ...
    return (type == ObjType_VNode_x86_64_pml4 ||
            ...
            type == ObjType_VNode_tilepro_l1 ||
            type == ObjType_VNode_tilepro_l2
           );
}
...
static inline size_t vnode_objbits(enum objtype type)
{
    ...
    else if(type == ObjType_VNode_tilepro_l1 ||
    		 type == ObjType_VNode_tilepro_l2)
    {
    	// XXX: here causes some confusion.
    	// this function is to return the size of page table,
    	// in fact tilepro's page table size (both L1 and L2) is 2KB, 2^11.
    	// so the real return value should be 11 bits, according to the function's requirement.
...
static inline size_t vnode_entry_bits(enum objtype type) {
    ...
    if (type == ObjType_VNode_tilepro_l1 ||
        type == ObjType_VNode_tilepro_l2)
    {
    	return 8; //HV_LOG2_L1_ENTRIES HV_LOG2_L2_ENTRIES
    }
