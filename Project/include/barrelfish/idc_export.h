...
/*
 * udn connect handler called by server export.
 * this function will mainly initialize binding object.
 * we use it to pass the destination core id and destination channel id of server.
 */
typedef errval_t udn_connect_callback_fn(void *st, struct monitor_binding *mb, uintptr_t mon_id, struct capref notify_cap, coreid_t dest_core_id, uint32_t out_chan_id, uint8_t dest_asid);
...
#endif
#ifdef CONFIG_INTERCONNECT_DRIVER_UDN
  udn_connect_callback_fn *udn_connect_callback;
#endif
};
