...
struct dispatcher_generic {
    ...
    /// List of UDN endpoints to poll
    struct udn_endpoint *udn_poll_list;

    /// List of UDN channels waiting to retry a send
    struct udn_chan *udn_send_events_list;

    /// UDN endpoint heap state
    struct heap udn_endpoint_heap;
