...
static errval_t spawn_setup_cspace(struct spawninfo *si)
{
    ...
    // here we define the same number as that in spawn_module() in startup.c
    int ram_num = 0;
#ifdef __tilepro__
    ram_num = 64;
#else
    ram_num = DEFAULT_CNODE_SLOTS;
#endif

    // Place the ram caps
    for (uint8_t i = 0; i < ram_num; i++) {
        struct capref base = {
            .cnode = basecn,
            .slot  = i
        };
...
static errval_t spawn_setup_vspace(struct spawninfo *si)
{
    ...
    case CPU_TILEPRO:
    	err = vnode_create(si->vtree, ObjType_VNode_tilepro_l1);
    	break;
...
errval_t spawn_free(struct spawninfo *si)
{
	...
	// When deleting the root-node, all other (except task) can't be deleted,
	// but that error will be ignored silently.
	// When deleting root-node after all others, monitor process won't start because,
	// capabilities are missing.
	DEBUG_ERR(12344321, "Some errors will follow, due to the implementation of this function, but they can be ignored.\n");
    ...
    DEBUG_ERR(12344321, "method finished.\n");
