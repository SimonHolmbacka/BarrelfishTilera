...
#if defined(CONFIG_INTERCONNECT_DRIVER_UMP) || defined(CONFIG_INTERCONNECT_DRIVER_UDN)
...
#ifdef CONFIG_INTERCONNECT_DRIVER_UDN
...
errval_t flounder_stub_udn_send_buf(struct flounder_udn_state *s, int msgnum, const void *bufp, size_t len, size_t *pos)
{
	const uint8_t *buf = bufp;
	...
	do {
		// can we send?
		if(!flounder_stub_udn_can_send(s)) {
			return FLOUNDER_ERR_BUF_SEND_MORE;
		}

		// construct a message
		msg = malloc(sizeof(struct udn_message));
		msg->type = msgnum;  // get the type
		...
		for( ; (msgpos < UDN_PAYLOAD_WORDS) && (*pos < len); msgpos++) {
			msg->data[msgpos] = getword(buf, pos, len);
		}
		...
		// send udn massage
		//debug_printf("sending udn string message...");
		udn_impl_send_msg(&s->chan.send_chan, msg);
		free(msg);
		// if there is some part of string left, continue to send
	} while(*pos < len);
...
errval_t flounder_stub_udn_recv_buf(struct udn_message *msg, void **bufp, size_t *len, size_t *pos)
{
	//debug_printf("received string message.");
...
errval_t flounder_stub_udn_send_string(struct flounder_udn_state *s, int msgnum, const char *str, size_t *pos, size_t *len)
{
    ...
    return flounder_stub_udn_send_buf(s, msgnum, str, *len, pos);
}

errval_t flounder_stub_udn_recv_string(struct udn_message *msg, char **strp, size_t *pos, size_t *len)
{
	return flounder_stub_udn_recv_buf(msg, (void**)strp, len, pos);
}

#endif // CONFIG_INTERCONNECT_DRIVER_UDN
