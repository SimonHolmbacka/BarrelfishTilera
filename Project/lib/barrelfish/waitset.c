...
#ifdef CONFIG_INTERCONNECT_DRIVER_UDN
/**
 * \brief Poll an incoming udn endpoint.
 * This is logically part of the udn endpoint implementation, but placed here
 * for easier inlining.
 */
static inline void udn_endpoint_poll(struct waitset_chanstate *chan)
{
    /* XXX: calculate location of endpoint from waitset channel state */
    struct udn_endpoint *ep = (struct udn_endpoint*)((char*)chan - offsetof(struct udn_endpoint, waitset_state));

    if (udn_endpoint_can_recv(ep)) {
        errval_t err = waitset_chan_trigger(chan);
        assert(err_is_ok(err)); // should not be able to fail
    }
}
...
#ifdef CONFIG_INTERCONNECT_DRIVER_UDN
    case CHANTYPE_UDN_IN:
        udn_endpoint_poll(chan);
        break;
#endif // CONFIG_INTERCONNECT_DRIVER_UDN
