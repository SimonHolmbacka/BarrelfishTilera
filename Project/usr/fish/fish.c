...
#ifdef __tilepro__
static void make_line(char* input, char* line) {
	int i;
...
static void getline(char *input, size_t size)
{
	...
	switch (call) {
	case 0:
		make_line(input, "help");
		break;
	case 1:
		make_line(input, "ps");
		break;
	case 2:
		make_line(input, "hellotest");
		// /tilepro/sbin/hellotest
		break;
	case 3:
		make_line(input, "demo");
		break;
	default:
		make_line(input, "poweroff");
		break;
	}
	call++;
#else
