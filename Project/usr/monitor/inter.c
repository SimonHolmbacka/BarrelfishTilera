...
static errval_t new_monitor_notify(coreid_t id)
{
    ...
    for (int i = 0; i < MAX_COREID; i++) {
        if (i != my_core_id && i != id) {
           ...
           // printf("new_monitor_notify1 my_id:%d function core_id:%d\n",(int)my_core_id,id);
            if (err_is_ok(err) && !notification_sent[min*MAX_COREID+max] && monitor_ready[i]) {
                while (1) {
                    ...
                    if (err_is_ok(err)) {
                        ...
                        printf("break my_core_id:%d id=%d i=%d\n",(int)my_core_id,id,i);
                        break;
                    }
                    printf("handel next message\n");
...
static void mem_serv_iref_request(struct intermon_binding *b)
{
    ...
    printf("reques mem_serv_iref: %d\n",(int)mem_serv_iref);
    printf("request core id %d \n",(int)my_core_id);
    printf("request binding: %d \n",(int)b);
...
{
	printf("aatest iref: %d and mem_serv_iref: %d\n",(int)iref,(int)mem_serv_iref);
	printf("reply core id %d \n",(int)my_core_id);
    printf("reply binding: %d \n",(int)b);
...
errval_t intermon_init(struct intermon_binding *b, coreid_t coreid)
{
    ...
    printf("udn_intermon_init aa on core:%d",(int)my_core_id);
    err = udn_intermon_init(b);
    if(err_is_fail(err)) {
    	USER_PANIC_ERR(err, "udn_intermon_init failed");
        return err;
    }
#endif
