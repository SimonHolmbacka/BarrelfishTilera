                             ...
                             uint32_t capid, uint8_t give_away)
{
    ...
    // XXX: sometimes this bool-type variable is used afterwards without being assigned value,
    // and in tilera simulator, the value is wrong and unpredictable,
    // so here we give it a initial value.
    // Don't know why it works with other architectures and even can be compiled?
...
errval_t monitor_server_init(struct monitor_binding *b)
{
    ...
    errval_t err3;
    err3 = udn_monitor_init(b);
    if (err_is_fail(err3)) {
        USER_PANIC_ERR(err3, "udn_monitor_init failed");
    }
#endif
