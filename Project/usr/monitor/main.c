...
static errval_t boot_app_core(int argc, char *argv[])
{
    ...
    printf("aa test (core %d ) to print binding pointer: %d \n",(int)my_core_id,(int)intermon_binding);
    // connect it to our request handlers
    intermon_init(intermon_binding, parent_core_id);

    /* Request memserv and nameserv iref */
#ifndef __scc__
    printf("\n aa debugpointer0 on core: %d \n ",(int)my_core_id);
    printf("core:%d",(int)my_core_id);
...
{
    printf("\naaa request_mem_serv_iref on core: %d\n",(int)my_core_id);
